...
const RecruitmentSection = ({ recruitmentSection }) => (
  <div className={styles.recruitmentSectionContainer}>
    ...
    {recruitmentSection.intro !== '' ? (
      <p className={styles.introTitle}>
        <span className={styles.boldIntro}>
          {recruitmentSection.shortTitle}
        </span>
        {recruitmentSection.intro}
      </p>
    ) : null}
    {recruitmentSection.studsLeaders.map(leader => (
      <RecruitmentLeaderSection key={leader.name} recruitmentLeader={leader} />
    ))}
  </div>
)
